...
const ContentForm = ({ model, closeModal }) => {
  ...
  const [currentTz, setCurrentTz] = useState("America/New_York");
  ...
  useEffect(() => {
    if (platforms.length) {
      if (state.PlatformId) {
        ...
        platforms.forEach(e => {
          if (e.id === state.PlatformId) {
            setCurrentTz(e.timezone || "America/New_York");
            moment.tz.setDefault(e.timezone);
          }
        });
      } else {
        setSelectedPlatformId(platforms[0].id);
        setCurrentTz(platforms[0].timezone);
      ...
      <div className="grid grid-cols-2 gap-8">
        <div>
          ...
          <div className="mt-6">
            ...
            <div className="mt-1 rounded-md shadow-sm relative">
              <Datetime
                ...
                value={(state && state.publishedDate) || ""}
                timezone={currentTz}
                name="publishedDate"
                onChange={event => onChange(event)}
              />
          ...
          <div className="mt-6">
            ...
            <div className="text-xs leading-4 mt-1 mb-2 text-cool-gray-500">
              Adding a start time will <strong> add a countdown </strong>for users view your content before it's live.
            </div>
            <div className="mt-1 rounded-md shadow-sm relative">
              <Datetime
